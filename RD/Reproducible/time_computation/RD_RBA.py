...
file_name = f'losses_rd_rba_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing Vanilla MLP with RBA-PINN for the diffusion-reaction equation...')
    layers = [2] + 4 * [20] + [1]

    model = PINN(X_u, u, X_f, lb, ub, savept=f"rd_rba_model_{seed}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'RD_RBA_Prediction_Results_{seed}.png'), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'RD_RBA_Loss_History_{seed}.png'), dpi=300)
    plt.show()
