...
import RD_C_LoReW_PINN as base
...
# Current defaults are Burgers: p=1.0, AC/KG: p=2.0, RD: p=8.0.
P_VALUES = [float(p) for p in range(1, 8)]
FIXED_KERNEL_H = 0.05
HP_CONFIGS = [
    {
...
def run_one_config(hp_config):
    ...
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'RD_C_LoReW_PINN_p_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_rd_clorew_p_sensitivity_{tag}_{seed}.txt'
    model_name = f'rd_clorew_p_sensitivity_{tag}_model_{seed}'
    ...
    log_message('Initializing C-LoReW-PINN p-sensitivity run on the reaction-diffusion equation...', train_log_path)
    log_message(
        'Config: h = %.5f, p = %.5f, seed = %d'
        % (hp_config['kernel_h'], hp_config['kernel_p'], seed),
        train_log_path,
    )
    log_message('Only kernel_p is changed from the base RD C-LoReW script; kernel_h is fixed at 0.05.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'rd_clorew_p_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
...
def plot_l2_comparison(results):
    ...
    plt.title(r'RD C-LoReW-PINN Sensitivity to $p$ ($h=0.05$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'RD_C-LoReW_p_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running C-LoReW-PINN p-sensitivity on the reaction-diffusion equation.')
    print('All settings are inherited from RD_C_LoReW_PINN.py except kernel_p.')
    print(f'Fixed kernel_h = {FIXED_KERNEL_H}')
    print(f'Seed: {seed}')
