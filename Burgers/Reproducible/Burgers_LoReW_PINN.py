...
file_name = f'losses_burgers_lorew_pinn_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing LoReW-PINN for the Burgers equation...')
    layers = [2] + 8 * [20] + [1]

    model = PINN(X_u, u, X_f, lb, ub, savept=f"burgers_lorew_pinn_model_{seed}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_LoReW_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_LoReW_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
