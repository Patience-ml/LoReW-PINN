...
file_name = f'losses_burgers_clorew_pinn_{seed}.txt'
...
try:
    ...
    raise FileNotFoundError(f'Cannot find burgers_shock.mat at {os.path.join(data_path, "burgers_shock.mat")}.')
...
# ==========================================
# 4. PINN with Kernel-Causal-Residual coupling
#    Keep the original C-LoReW style:
...
if __name__ == "__main__":
    ...
    log_message('Initializing C-LoReW-PINN for the Burgers equation...')
    layers = [2] + 8 * [20] + [1]

    model = PINN(X_u, u, X_f, lb, ub, savept=f"burgers_clorew_pinn_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (C-LoReW-PINN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_C_LoReW_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (C-LoReW-PINN)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_C_LoReW_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
