...
file_name = f'losses_burgers_sa_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing SA-PINN for the Burgers equation...')
    layers = [2] + 8 * [20] + [1]

    model = PINN(X_u, u, X_f, lb, ub, savept=f"burgers_sa_model_{seed}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_SA_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_SA_Loss_History_{seed}.png"), dpi=300)
    plt.show()
