...
file_name = f'losses_burgers_pinn_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing PINN for the Burgers equation...')
    layers = [2] + 8 * [20] + [1]

    model = PINN(X_u, u, X_f, lb, ub, savept=f"burgers_pinn_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (PINN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (PINN)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Burgers_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
