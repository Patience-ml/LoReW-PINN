...
def run_one_config(hp_config):
    ...
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'Burgers_C_LoReW_PINN_p_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_burgers_clorew_p_sensitivity_{tag}_{seed}.txt'
    model_name = f'burgers_clorew_p_sensitivity_{tag}_model_{seed}'
    ...
    log_message('Initializing C-LoReW-PINN p-sensitivity run on the Burgers equation...', train_log_path)
    log_message(
        'Config: h = %.5f, p = %.5f, seed = %d'
        % (hp_config['kernel_h'], hp_config['kernel_p'], seed),
        train_log_path,
    )
    log_message('Only kernel_p is changed from the base Burgers C-LoReW script; kernel_h is fixed at 0.10.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'burgers_clorew_p_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
...
def plot_l2_comparison(results):
    ...
    plt.title(r'Burgers C-LoReW-PINN Sensitivity to $p$ ($h=0.10$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'Burgers_C-LoReW_p_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running C-LoReW-PINN p-sensitivity on the Burgers equation.')
    print('All settings are inherited from Burgers_C_LoReW_PINN.py except kernel_p.')
