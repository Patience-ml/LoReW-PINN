...
file_name = f'losses_helmholtz_rba_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing RBA-PINN for the Helmholtz equation...')

    model = PINN(X_u, u, X_r, lb, ub, savept=f"helmholtz_rba_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,y) (RBA-PINN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Helmholtz_RBA_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (RBA-PINN)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Helmholtz_RBA_Loss_History_{seed}.png"), dpi=300)
    plt.show()
