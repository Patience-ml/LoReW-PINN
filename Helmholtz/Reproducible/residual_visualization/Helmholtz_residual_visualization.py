...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Helmholtz residual-evolution heatmaps for Vanilla PINN, "
            "SA-PINN, RBA-PINN, and LoReW-PINN."
        )
    )
    parser.add_argument("--checkpoints", type=int, nargs="+", default=[1000, 5000, 15000])
    parser.add_argument("--grid-size", type=int, default=201)
    parser.add_argument("--train-grid-size", type=int, default=101)
    parser.add_argument("--n-boundary-each", type=int, default=50)
    parser.add_argument("--seed", type=int, default=5472)
    ...
    parser.add_argument("--output-name", type=str, default="helmholtz_residual_evolution")
    ...
    parser.add_argument("--lorew-eta", type=float, default=5e-4)
    parser.add_argument("--lorew-gamma", type=float, default=0.98)
    parser.add_argument("--kernel-h-x", type=float, default=0.12)
    parser.add_argument("--kernel-h-y", type=float, default=0.12)
    parser.add_argument("--kernel-p", type=float, default=1.5)
    parser.add_argument("--kernel-trunc", type=float, default=3.0)
    parser.add_argument("--num-kernel-centers-x", type=int, default=31)
    parser.add_argument("--num-kernel-centers-y", type=int, default=31)
...
def exact_u_np(x: np.ndarray, y: np.ndarray, cfg: Config) -> np.ndarray:
    return np.sin(cfg.xdim * np.pi * x) * np.sin(cfg.ydim * np.pi * y)


def source_torch(x: torch.Tensor, y: torch.Tensor, cfg: Config) -> torch.Tensor:
    exact = torch.sin(cfg.xdim * np.pi * x) * torch.sin(cfg.ydim * np.pi * y)
    return -((cfg.xdim * np.pi) ** 2) * exact - ((cfg.ydim * np.pi) ** 2) * exact + cfg.ksq * exact


def make_problem(cfg: Config) -> Dict[str, np.ndarray]:
    x0 = np.linspace(cfg.x_min, cfg.x_max, cfg.grid_size)[:, None]
    y0 = np.linspace(cfg.y_min, cfg.y_max, cfg.grid_size)[:, None]
    X, Y = np.meshgrid(x0.flatten(), y0.flatten(), indexing="ij")
    Exact = exact_u_np(X, Y, cfg)
    X_star = np.hstack((X.flatten()[:, None], Y.flatten()[:, None]))

    train_x = np.linspace(cfg.x_min, cfg.x_max, cfg.train_grid_size)[:, None]
    train_y = np.linspace(cfg.y_min, cfg.y_max, cfg.train_grid_size)[:, None]
    X_r_grid, Y_r_grid = np.meshgrid(train_x.flatten(), train_y.flatten(), indexing="ij")
    X_r = np.hstack((X_r_grid.flatten()[:, None], Y_r_grid.flatten()[:, None]))

    boundary_x = np.linspace(cfg.x_min, cfg.x_max, cfg.n_boundary_each)[:, None]
    boundary_y = np.linspace(cfg.y_min, cfg.y_max, cfg.n_boundary_each)[:, None]
    X_left = np.hstack((np.full((cfg.n_boundary_each, 1), cfg.x_min), boundary_y))
    X_right = np.hstack((np.full((cfg.n_boundary_each, 1), cfg.x_max), boundary_y))
    X_top = np.hstack((boundary_x, np.full((cfg.n_boundary_each, 1), cfg.y_max)))
    X_bottom = np.hstack((boundary_x, np.full((cfg.n_boundary_each, 1), cfg.y_min)))
...
class BaseRunner:
    ...
    def net_u(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        return self.dnn(torch.cat([x, y], dim=1))

    def net_r(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        u = self.net_u(x, y)
        u_y = grad(u, y)
        u_yy = grad(u_y, y)
        u_x = grad(u, x)
        u_xx = grad(u_x, x)
        return u_xx + u_yy + self.cfg.ksq * u - source_torch(x, y, self.cfg)
    ...
    def residual_grid(
        ...
        for start in range(0, X_star.shape[0], chunk_size):
            ...
            y = torch.tensor(pts[:, 1:2], requires_grad=True).float().to(self.device)
            r = self.net_r(x, y)
...
class SAPINNRunner(BaseRunner):
    ...
    def __init__(self, cfg: Config, device: torch.device, X_u, u, X_r):
        ...
        lamu = torch.rand(self.N_u, 1).float().to(device) * 1.0
...
class RBAPINNRunner(BaseRunner):
    ...
    def train_step(self) -> None:
        ...
        r_norm = self.cfg.rba_eta * abs_r_detach / (torch.max(abs_r_detach) + self.cfg.kernel_eps)
...
class LoReWPINNRunner(BaseRunner):
    name = "LoReW-PINN"

    def __init__(self, cfg: Config, device: torch.device, X_u, u, X_r):
        super().__init__(cfg, device, X_u, u, X_r)
        self.rsum = torch.zeros((self.N_r, 1), device=device)
        self.x_centers = torch.linspace(cfg.x_min, cfg.x_max, cfg.num_kernel_centers_x, device=device).view(1, -1)
        self.y_centers = torch.linspace(cfg.y_min, cfg.y_max, cfg.num_kernel_centers_y, device=device).view(1, -1)
    ...
    def build_kernel_weights(self, x_points: torch.Tensor, y_points: torch.Tensor) -> torch.Tensor:
        dx = x_points - self.x_centers
        dy = y_points - self.y_centers
        Kx = torch.exp(-0.5 * (dx / self.cfg.kernel_h_x) ** 2)
        Ky = torch.exp(-0.5 * (dy / self.cfg.kernel_h_y) ** 2)
        if self.cfg.kernel_trunc is not None and self.cfg.kernel_trunc > 0:
            Kx = Kx * (torch.abs(dx) <= self.cfg.kernel_trunc * self.cfg.kernel_h_x).float()
            Ky = Ky * (torch.abs(dy) <= self.cfg.kernel_trunc * self.cfg.kernel_h_y).float()
        ...
        if torch.any(zero_rows):
            center_x = self.x_centers.T.repeat(1, self.cfg.num_kernel_centers_y).reshape(1, -1)
            center_y = self.y_centers.repeat(self.cfg.num_kernel_centers_x, 1).reshape(1, -1)
            dist2 = ((x_points[zero_rows] - center_x) / self.cfg.kernel_h_x) ** 2
            dist2 = dist2 + ((y_points[zero_rows] - center_y) / self.cfg.kernel_h_y) ** 2
    ...
    def train_step(self) -> None:
        ...
        r_norm = self.cfg.lorew_eta * abs_r_detach / (local_scale + self.cfg.kernel_eps)
        self.rsum = (self.rsum * self.cfg.lorew_gamma + r_norm).detach()
        loss_r = torch.mean((self.rsum * r_pred) ** 2)
        loss_u = torch.mean((u_pred - self.u) ** 2)
        loss = loss_r + loss_u
        loss.backward()
...
def build_config(args: argparse.Namespace) -> Config:
    ...
    return Config(
        ...
        n_boundary_each=args.n_boundary_each,
        ...
        lorew_eta=args.lorew_eta,
        lorew_gamma=args.lorew_gamma,
        kernel_h_x=args.kernel_h_x,
        kernel_h_y=args.kernel_h_y,
        kernel_p=args.kernel_p,
        kernel_trunc=args.kernel_trunc,
        num_kernel_centers_x=args.num_kernel_centers_x,
        num_kernel_centers_y=args.num_kernel_centers_y,
...
def plot_residual_evolution(
    ...
    x0 = problem["x0"].flatten()
    y0 = problem["y0"].flatten()
    exact = problem["Exact"]
    ...
    np.savez_compressed(
        ...
        x=problem["x0"].flatten(),
        y=problem["y0"].flatten(),
        exact=problem["Exact"],
...
def main() -> None:
    ...
    grid_shape = (cfg.grid_size, cfg.grid_size)
    ...
    for method_name, runner_cls in runner_specs:
        ...
        runner = runner_cls(cfg, device, problem["X_u"], problem["u"], problem["X_r"])
