...
file_name = f'losses_helmholtz_sa_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message('Initializing SA-PINN for the Helmholtz equation...')

    model = PINN(X_u, u, X_r, lb, ub, savept=f"helmholtz_sa_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,y) (SA-PINN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Helmholtz_SA_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Helmholtz_SA_Loss_History_{seed}.png"), dpi=300)
    plt.show()
