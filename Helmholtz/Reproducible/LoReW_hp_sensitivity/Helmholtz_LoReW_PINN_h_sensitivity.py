...
def run_one_config(hp_config):
    ...
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'Helmholtz_LoReW_PINN_h_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_helmholtz_lorew_h_sensitivity_{tag}_{seed}.txt'
    model_name = f'helmholtz_lorew_h_sensitivity_{tag}_model_{seed}'
    ...
    log_message('Only kernel_h_x/kernel_h_y are changed from the base LoReW script; kernel_p is fixed at 1.5.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'helmholtz_lorew_h_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
...
def plot_l2_comparison(results):
    ...
    plt.title(r'Helmholtz LoReW-PINN Sensitivity to $h$ ($p=1.5$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'Helmholtz_LoReW_h_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running LoReW-PINN h-sensitivity on the Helmholtz equation.')
    print('All settings are inherited from Helmholtz_LoReW_PINN.py except kernel_h_x/kernel_h_y.')
