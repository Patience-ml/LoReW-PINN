...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Klein-Gordon residual-evolution heatmaps for Vanilla PINN, "
    ...
    parser.add_argument("--n-residual", type=int, default=1000)
    ...
    parser.add_argument("--output-name", type=str, default="kg_residual_evolution")
    ...
    parser.add_argument(
        ...
        default=None,
        help="Subset of methods to run. By default all methods are included.",
    )
    ...
    parser.add_argument("--clorew-eta", type=float, default=5e-4)
    parser.add_argument("--clorew-gamma", type=float, default=0.997)
    ...
    parser.add_argument("--rho-g", type=float, default=0.99)
    parser.add_argument("--kernel-h", type=float, default=0.02)
...
def exact_ut_np(x: np.ndarray, t: np.ndarray) -> np.ndarray:
    return -5.0 * np.pi * x * np.sin(5.0 * np.pi * t) + 3.0 * (x**3) * (t**2)


def source_torch(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
...
def make_problem(cfg: Config) -> Dict[str, np.ndarray]:
    ...
    x_u = np.linspace(cfg.x_min, cfg.x_max, cfg.n_initial)[:, None]
    X_u = np.hstack((x_u, np.zeros_like(x_u)))
    u_u = exact_u_np(X_u[:, 0:1], X_u[:, 1:2])
    ut_u = exact_ut_np(X_u[:, 0:1], X_u[:, 1:2])

    t_b = np.linspace(cfg.t_min, cfg.t_max, cfg.n_boundary)[:, None]
    X_lb = np.hstack((np.full((cfg.n_boundary, 1), cfg.x_min), t_b))
    X_ub = np.hstack((np.full((cfg.n_boundary, 1), cfg.x_max), t_b))
...
class BaseRunner:
    ...
    def __init__(self, cfg: Config, device: torch.device, problem: Dict[str, np.ndarray]):
        ...
        self.x_u = torch.tensor(problem["X_u"][:, 0:1], requires_grad=True).float().to(device)
        self.t_u = torch.tensor(problem["X_u"][:, 1:2], requires_grad=True).float().to(device)
        self.x_r = torch.tensor(problem["X_f"][:, 0:1], requires_grad=True).float().to(device)
        self.t_r = torch.tensor(problem["X_f"][:, 1:2], requires_grad=True).float().to(device)
        self.x_lb = torch.tensor(problem["X_lb"][:, 0:1], requires_grad=True).float().to(device)
        self.t_lb = torch.tensor(problem["X_lb"][:, 1:2], requires_grad=True).float().to(device)
        self.x_ub = torch.tensor(problem["X_ub"][:, 0:1], requires_grad=True).float().to(device)
        self.t_ub = torch.tensor(problem["X_ub"][:, 1:2], requires_grad=True).float().to(device)

        self.u = torch.tensor(problem["u_u"]).float().to(device)
        self.ut = torch.tensor(problem["ut_u"]).float().to(device)
        self.u_lb = torch.tensor(problem["u_lb"]).float().to(device)
        self.u_ub = torch.tensor(problem["u_ub"]).float().to(device)
        self.N_r = self.x_r.shape[0]
        self.N_u = self.u.shape[0]
        self.N_b = self.u_lb.shape[0]
    ...
    def net_r(self, x: torch.Tensor, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        u = self.net_u(x, t)
        u_t = grad(u, t)
        u_tt = grad(u_t, t)
        u_x = grad(u, x)
        u_xx = grad(u_x, x)
        residual = u_tt - u_xx + u**3 - source_torch(x, t)
        return residual, u_t
    ...
    def supervised_losses(self):
        ...
        loss_b = 0.5 * (
            torch.mean((u_lb_pred - self.u_lb) ** 2)
            + torch.mean((u_ub_pred - self.u_ub) ** 2)
        )
        return loss_u, loss_ut, loss_b
    ...
    def train_and_collect_residuals(
        ...
        for _ in range(max_steps):
            ...
            if self.iter in checkpoint_set:
                ...
                print(
                    f"  {self.name}: collected residuals at iteration {self.iter} ({elapsed:.1f}s)",
                    flush=True,
                )
...
class VanillaPINNRunner(BaseRunner):
    ...
    def train_step(self) -> None:
        ...
        loss_u, loss_ut, loss_b = self.supervised_losses()
...
class SAPINNRunner(BaseRunner):
    ...
    def __init__(self, cfg: Config, device: torch.device, problem: Dict[str, np.ndarray]):
        ...
        lamut = torch.rand(self.N_u, 1).float().to(device) * 100.0
        ...
        self.optimizer2 = torch.optim.Adam(
            [self.lamr, self.lamu, self.lamut, self.lamb],
            lr=cfg.sa_weight_lr,
            maximize=True,
        )
...
class RBAPINNRunner(BaseRunner):
    ...
    def train_step(self) -> None:
        ...
        r_norm = self.cfg.rba_eta * abs_r.detach() / (torch.max(abs_r.detach()) + self.cfg.kernel_eps)
...
def main() -> None:
    ...
    all_runner_specs = [
        ("vanilla", "Vanilla PINN", VanillaPINNRunner),
        ("sa", "SA-PINN", SAPINNRunner),
        ("rba", "RBA-PINN", RBAPINNRunner),
        ("clorew", "C-LoReW-PINN", CLoReWPINNRunner),
    ]
    requested_methods = args.methods
    if requested_methods is None:
        requested_methods = ["vanilla", "sa", "rba", "clorew"]
    if args.no_vanilla:
        requested_methods = [name for name in requested_methods if name != "vanilla"]
    runner_specs = [spec for spec in all_runner_specs if spec[0] in requested_methods]

    all_residuals = {}
    checkpoint_key = "-".join(str(step) for step in checkpoints)
    for method_key, method_name, runner_cls in runner_specs:
        cache_path = os.path.join(
            data_dir,
            f"{args.output_name}_{method_key}_seed{args.seed}_steps{checkpoint_key}.npz",
        )
        ...
        print(f"Training {method_name} and collecting residual fields...", flush=True)
        ...
        np.savez_compressed(
            ...
            checkpoints=np.array(checkpoints),
            residual_maps=np.array(
                [all_residuals[method_name][step] for step in checkpoints],
                dtype=np.float64,
            ),
        )
        print(f"Saved method cache: {cache_path}", flush=True)
