...
file_name = f'losses_kg_pinn_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message("Initializing PINN for the Klein-Gordon equation...")
    layers = [2] + 5 * [50] + [1]

    model = PINN(X_u, u_u, ut_u, X_f, X_lb, X_ub, u_lb, u_ub, lb, ub, savept=f"kg_pinn_model_{seed}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"KG_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"KG_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
