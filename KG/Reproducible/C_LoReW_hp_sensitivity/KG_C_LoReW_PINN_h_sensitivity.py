...
def run_one_config(hp_config):
    ...
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'KG_C_LoReW_PINN_h_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_kg_clorew_h_sensitivity_{tag}_{seed}.txt'
    model_name = f'kg_clorew_h_sensitivity_{tag}_model_{seed}'
    ...
    log_message('Initializing C-LoReW-PINN h-sensitivity run on the Klein-Gordon equation...', train_log_path)
    log_message(
        'Config: h = %.5f, p = %.5f, seed = %d'
        % (hp_config['kernel_h'], hp_config['kernel_p'], seed),
        train_log_path,
    )
    log_message('Only kernel_h is changed from the base KG C-LoReW script; kernel_p is fixed at 2.0.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'kg_clorew_h_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
...
def plot_l2_comparison(results):
    ...
    plt.title(r'KG C-LoReW-PINN Sensitivity to $h$ ($p=2.0$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'KG_C-LoReW_h_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running C-LoReW-PINN h-sensitivity on the Klein-Gordon equation.')
    print('All settings are inherited from KG_C_LoReW_PINN.py except kernel_h.')
