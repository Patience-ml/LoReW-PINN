...
file_name = f'losses_kg_lorew_pinn_{seed}.txt'
...
# ==========================================
# 4. PINN with Kernel-Residual coupling
#    Keep the corresponding method configuration while removing only the causal gate:
...
if __name__ == "__main__":
    ...
    log_message("Initializing LoReW-PINN for the Klein-Gordon equation...")
    layers = [2] + 5 * [50] + [1]

    model = PINN(X_u, u_u, ut_u, X_f, X_lb, X_ub, u_lb, u_ub, lb, ub, savept=f"kg_lorew_pinn_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (LoReW-PINN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"KG_LoReW_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"KG_LoReW_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
