...
def get_output_names(hp_config):
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'AC_C_LoReW_fourier_p_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_ac_clorew_fourier_p_sensitivity_{tag}_{seed}.txt'
    model_name = f'ac_clorew_fourier_p_sensitivity_{tag}_model_{seed}'
...
def run_one_config(hp_config):
    ...
    log_message('Initializing C-LoReW-Fourier p-sensitivity run on the Allen-Cahn equation...', train_log_path)
    log_message(
        'Config: h = %.5f, p = %.5f, seed = %d'
        % (hp_config['kernel_h'], hp_config['kernel_p'], seed),
        train_log_path,
    )
    log_message('Only kernel_p is changed from the base AC C-LoReW-Fourier script; kernel_h is fixed at 0.05.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'ac_clorew_fourier_p_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
...
def plot_l2_comparison(results):
    ...
    plt.title(r'AC C-LoReW-Fourier Sensitivity to $p$ ($h=0.05$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'AC_C_LoReW_fourier_p_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running C-LoReW-Fourier p-sensitivity on the Allen-Cahn equation.')
    print('All settings are inherited from AC_C_LoReW_fourier.py except kernel_p.')
