...
DATA_PATH = os.path.abspath(os.path.join(FILE_DIR, '..', 'AC.mat'))
...
METHODS = [
    {
        'name': 'Vanilla PINN',
        'model_file': f'ac_pinn_fourier_model_{SEED}.pt',
    },
    {
        'name': 'SA-PINN',
        'model_file': f'ac_sa_fourier_model_{SEED}.pt',
    },
    {
        'name': 'RBA-PINN',
        'model_file': f'ac_rba_fourier_model_{SEED}.pt',
    },
    {
        'name': 'C-LoReW-PINN',
        'model_file': f'ac_clorew_fourier_model_{SEED}.pt',
    },
]
...
def load_ac_data():
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(f'Cannot find AC.mat at {DATA_PATH}')
...
def load_prediction(model_file, x_star, output_shape):
    ...
    model = Fourier(LAYERS).to(DEVICE)
    ...
    nt, nx = output_shape[1], output_shape[0]
...
def add_panel_label(ax, label, y=-0.30):
    ax.text(
        0.5,
        y,
        label,
...
def main(show=True):
    ...
    for col, method in enumerate(METHODS, start=1):
        ...
        err_ax.set_title(
            r'Absolute error: $\vert u-\hat{u}\vert$',
            fontsize=13,
            pad=8,
        )
        ...
        add_panel_label(err_ax, method_label, y=-0.34)
    ...
    output_png = os.path.join(FIGURE_DIR, 'AC_plotting_comparison.png')
    output_pdf = os.path.join(FIGURE_DIR, 'AC_plotting_comparison.pdf')
