...
file_name = f'losses_lorew_pinn_fourier_{seed}.txt'
...
# ==========================================
# 4. PINN with Fourier features and Kernel-Residual coupling
#    Keep the corresponding Fourier-method configuration while removing only the causal gate:
#    residual-normalization -> rsum -> weighted residual loss
# ==========================================
class PINN():
...
if __name__ == "__main__":
    ...
    log_message("Initializing Fourier-feature MLP with LoReW-PINN...")
    ...
    model = PINN(X_u, u, X_f, lb, ub, dimx, dimt, savept=f"ac_lorew_fourier_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (LoReW-Fourier)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_LoReW_PINN_Fourier_Prediction_Results_{seed}.png'), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (LoReW-Fourier)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_LoReW_PINN_Fourier_Loss_History_{seed}.png'), dpi=300)
    plt.show()
