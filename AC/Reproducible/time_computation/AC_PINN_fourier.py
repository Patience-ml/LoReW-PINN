...
file_name = f'losses_pinn_fourier_{seed}.txt'
...
class PINN():
    ...
    def loss_func(self):
        ...
        loss_u = self.ic_weight * torch.mean((self.u_pred - self.u)**2)
...
if __name__ == "__main__":
    ...
    log_message("Initializing Fourier-feature MLP with PINN...")
    ...
    model = PINN(X_u, u, X_f, lb, ub, dimx, dimt, savept=f"ac_pinn_fourier_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (PINN-Fourier)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_PINN_Fourier_Prediction_Results_{seed}.png'), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (PINN-Fourier)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_PINN_Fourier_Loss_History_{seed}.png'), dpi=300)
    plt.show()
