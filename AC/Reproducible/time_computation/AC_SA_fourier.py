...
file_name = f'losses_sa_fourier_{seed}.txt'
...
try:
    ...
    raise FileNotFoundError(f'Cannot find AC.mat at {os.path.join(data_path, "AC.mat")}.')
...
class PINN():
    def __init__(self, X_u, u, X_r, lb, ub, dimx, dimt, savept=None):
        ...
        if self.sa == 1:
            lamr = torch.rand(self.N_r, 1, requires_grad=True).float().to(device) * 1
            lamu = torch.rand(self.N_u, 1, requires_grad=True).float().to(device) * 100
...
if __name__ == "__main__":
    ...
    log_message('Initializing Fourier-feature MLP with SA-PINN...')
    log_message(f'Initial-condition samples N_u = {N_u}')
    log_message(f'Initial adaptive condition weight scale = {IC_WEIGHT:g}')
    layers = [2] + 6 * [128] + [1]
    dimx = N_u
    dimt = N_u

    model = PINN(X_u, u, X_f, lb, ub, dimx, dimt, savept=f"ac_sa_fourier_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (SA-Fourier)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_SA_Fourier_Prediction_Results_{seed}.png'), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (SA-Fourier)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_SA_Fourier_Loss_History_{seed}.png'), dpi=300)
    plt.show()
