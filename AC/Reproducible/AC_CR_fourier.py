...
file_name = f'losses_cr_fourier_{seed}.txt'
...
# ==========================================
# 4. PINN with Fourier features and Causal-Residual weighting
#    Residual weighting follows RBA:
...
class PINN():
    def __init__(self, X_u, u, X_r, lb, ub, dimx, dimt, savept=None):
        ...
        if self.rba == 1:
            ...
            # Gate-weighted mean absolute residual for tau update
...
if __name__ == "__main__":
    ...
    log_message("Initializing Fourier-feature MLP with CR-PINN...")
    ...
    model = PINN(X_u, u, X_f, lb, ub, dimx, dimt, savept=f"ac_cr_fourier_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,t) (CR-Fourier)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_CR_Fourier_Prediction_Results_{seed}.png'), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (CR-Fourier)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f'AC_CR_Fourier_Loss_History_{seed}.png'), dpi=300)
    plt.show()
