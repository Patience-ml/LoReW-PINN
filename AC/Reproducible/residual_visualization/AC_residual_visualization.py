...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Allen-Cahn residual-evolution heatmaps for Vanilla PINN, "
            "SA-PINN, RBA-PINN, and C-LoReW-PINN."
        )
    )
    parser.add_argument("--checkpoints", type=int, nargs="+", default=[2000, 20000, 300000])
    parser.add_argument("--seed", type=int, default=2832)
    parser.add_argument("--n-initial", type=int, default=256)
    parser.add_argument("--n-residual", type=int, default=25600)
    ...
    parser.add_argument("--fourier-period", type=float, default=2.0)
    parser.add_argument("--fourier-modes", type=int, default=10)
    parser.add_argument("--chunk-size", type=int, default=4096)
    parser.add_argument("--device", choices=["auto", "cpu", "cuda"], default="auto")
    parser.add_argument("--output-name", type=str, default="ac_residual_evolution")
    ...
    parser.add_argument("--eta-g", type=float, default=1e-4)
...
def make_problem(cfg: Config, script_dir: Path) -> Dict[str, np.ndarray]:
    data_path = script_dir.parent.parent / "AC.mat"
    if not data_path.exists():
        raise FileNotFoundError(f"Cannot find Allen-Cahn data file: {data_path}")
    ...
    idx_x = np.random.choice(x.shape[0], cfg.n_initial, replace=False)
    x_u = x[idx_x, :]
    t_u = np.zeros((cfg.n_initial, 1))
    u_u = exact[idx_x, 0:1]
    x_u_all = np.hstack((x_u, t_u))

    lb = x_star.min(0)
    ub = x_star.max(0)
    x_f = lb + (ub - lb) * latin_hypercube(2, cfg.n_residual, cfg.seed)
    x_f = np.vstack((x_f, x_u_all))
...
class Fourier(torch.nn.Module):
    def __init__(self, layers: Tuple[int, ...], period: float, modes: int):
        super().__init__()
        self.L = period
        self.M = modes
        fourier_layers = list(layers)
        fourier_layers[0] = 2 * self.M + 2
        self.model = DNN(tuple(fourier_layers))
        self.register_buffer("k", torch.arange(1, self.M + 1).float().view(1, -1))

    def encoding(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        w = 2.0 * np.pi / self.L
        return torch.hstack(
            [
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        x = h[:, 0:1]
        t = h[:, 1:2]
        return self.model(self.encoding(x, t))
...
class BaseRunner:
    def __init__(self, cfg: Config, device: torch.device, problem: Dict[str, np.ndarray]):
        ...
        self.iter = 0
        self.dnn = Fourier(cfg.layers, cfg.fourier_period, cfg.fourier_modes).to(device)
    ...
    def loss_step(self) -> None:
        ...
        loss = torch.mean(r_pred**2) + self.cfg.ic_weight * torch.mean((u_pred - self.u) ** 2)
        loss.backward()
        self.optimizer.step()
...
class SAPINNRunner(BaseRunner):
    ...
    def loss_step(self) -> None:
        ...
        loss = loss_r + self.cfg.ic_weight * loss_u
        loss.backward()
        self.optimizer.step()
...
class RBAPINNRunner(BaseRunner):
    ...
    def loss_step(self) -> None:
        ...
        loss = torch.mean((self.rsum * r_pred) ** 2) + self.cfg.ic_weight * torch.mean((u_pred - self.u) ** 2)
        loss.backward()
        self.optimizer.step()
...
class CLoReWPINNRunner(BaseRunner):
    ...
    def loss_step(self) -> None:
        ...
        self.rsum = (self.rsum * self.cfg.clorew_gamma + r_norm * self.gbar).detach()
        loss = torch.mean((self.rsum * r_pred) ** 2) + self.cfg.ic_weight * torch.mean((u_pred - self.u) ** 2)
        loss.backward()
        self.optimizer.step()
        ...
        self.tau_gate = self.tau_gate + self.cfg.eta_g * np.exp(-self.cfg.epsilon * gate_abs_res.item() / self.gate_abs_res_ref)
...
def plot_residual_evolution(
    ...
    panel_limits = compute_limits(value_maps, method_names, checkpoints, args.scale_mode, args.clip_percentiles)
    fig, axes = plt.subplots(
        len(method_names),
        len(checkpoints),
        figsize=(max(5.0 * len(checkpoints), 9.0), max(3.35 * len(method_names), 6.0)),
        squeeze=False,
    )
    extent = [
        float(problem["t"].min()),
...
def main() -> None:
    ...
    specs = [
        ("vanilla", "Vanilla PINN", BaseRunner, "ac_pinn_fourier_model_{seed}.pt"),
        ("sa", "SA-PINN", SAPINNRunner, "ac_sa_fourier_model_{seed}.pt"),
        ("rba", "RBA-PINN", RBAPINNRunner, "ac_rba_fourier_model_{seed}.pt"),
        ("clorew", "C-LoReW-PINN", CLoReWPINNRunner, "ac_clorew_fourier_model_{seed}.pt"),
    ]
    ...
    plot_residual_evolution(all_residuals, checkpoints, problem, args, figure_dir, data_dir)
