...
METHODS = [
    {
        'name': 'Vanilla PINN',
        'model_file': f'poisson_pinn_model_{SEED}.pt',
    },
    {
        'name': 'SA-PINN',
        'model_file': f'poisson_sa_model_{SEED}.pt',
    },
    {
        'name': 'RBA-PINN',
        'model_file': f'poisson_rba_model_{SEED}.pt',
    },
    {
        'name': 'LoReW-PINN',
        'model_file': f'poisson_lorew_pinn_model_{SEED}.pt',
    },
]
...
def load_poisson_data():
    ...
    x_grid, y_grid = np.meshgrid(x.flatten(), y.flatten(), indexing='ij')

    exact = exact_u_np(x_grid, y_grid)
...
def main(show=True):
    x, y, x_star, exact = load_poisson_data()
    ...
    output_png = os.path.join(FIGURE_DIR, 'Poisson_plotting_comparison.png')
    output_pdf = os.path.join(FIGURE_DIR, 'Poisson_plotting_comparison.pdf')
