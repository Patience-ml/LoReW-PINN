...
def run_one_config(hp_config):
    ...
    tag = hp_config['tag']
    train_log_path = os.path.join(LOG_DIR, f'Poisson_LoReW_PINN_p_sensitivity_{tag}_train_log_{seed}.txt')
    loss_file_name = f'losses_poisson_lorew_p_sensitivity_{tag}_{seed}.txt'
    model_name = f'poisson_lorew_p_sensitivity_{tag}_model_{seed}'
    ...
    log_message('Only kernel_p is changed from the base LoReW script; kernel_h_x/kernel_h_y are fixed at 0.05.', train_log_path)
...
def save_summary(results):
    summary_path = os.path.join(LOG_DIR, f'poisson_lorew_p_sensitivity_summary_{seed}.csv')
    fieldnames = [
        'tag',
        'seed',
        'kernel_h',
    ...
    print('Best Relative L2 Error for each p value:')
    for row in sorted(results, key=lambda item: item['kernel_p']):
        print(
            'p=%.5f | h=%.5f, best_L2RE=%.5e, best_iter=%d, final_L2RE=%.5e, time=%s'
            % (
                row['kernel_p'],
                row['kernel_h'],
                row['best_l2_rel'],
    ...
    print(
        'Best overall: p=%.5f, h=%.5f, best_L2RE=%.5e at iter %d'
        % (
            best_row['kernel_p'],
            best_row['kernel_h'],
...
def plot_l2_comparison(results):
    ...
    plt.title(r'Poisson LoReW-PINN Sensitivity to $p$ ($h=0.05$)', fontsize=14)
    plt.legend(fontsize=8, ncol=3)
    plt.tight_layout()
    output_path = os.path.join(FIGURE_DIR, f'Poisson_LoReW_p_sensitivity_L2_{seed}.png')
...
if __name__ == "__main__":
    print('Running LoReW-PINN p-sensitivity on the Poisson equation.')
    print('All settings are inherited from Poisson_LoReW_PINN.py except kernel_p.')
