...
file_name = f'losses_poisson_rba_{seed}.txt'
...
class PINN():
    ...
    def loss_func(self):
        ...
        if self.rba == 1:
            ...
            r_norm = eta * torch.abs(self.r_pred).detach() / (torch.max(torch.abs(self.r_pred).detach()) + self.rba_eps)
        ...
        if self.iter % self.print_step == 0:
            with torch.no_grad():
                ...
                log_message('Iter %d, Loss: %.3e, Rel_L2: %.3e, L_inf: %.3e, rsum_mean: %.3e, Time: %s' %
                            (self.iter, self.loss.item(), l2_rel, l_inf, rsum_mean, format_elapsed_time(self.exec_time)))
...
if __name__ == "__main__":
    ...
    log_message("Initializing RBA-PINN for the Poisson equation...")
    layers = [2] + 8 * [50] + [1]

    model = PINN(X_b, u_b, X_f, lb, ub, savept=f"poisson_rba_model_{seed}")
    ...
    ax[1].set_title('Predicted u(x,y) (RBA)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Poisson_RBA_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.title('Convergence History (RBA)', fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Poisson_RBA_Loss_History_{seed}.png"), dpi=300)
    plt.show()
