...
file_name = f'losses_poisson_pinn_{seed}.txt'
...
if __name__ == "__main__":
    ...
    log_message("Initializing PINN for the Poisson equation...")
    layers = [2] + 8 * [50] + [1]

    model = PINN(X_b, u_b, X_f, lb, ub, savept=f"poisson_pinn_model_{seed}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Poisson_PINN_Prediction_Results_{seed}.png"), dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURE_DIR, f"Poisson_PINN_Loss_History_{seed}.png"), dpi=300)
    plt.show()
