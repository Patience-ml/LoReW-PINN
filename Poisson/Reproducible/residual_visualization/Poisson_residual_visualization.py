...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Poisson residual-evolution heatmaps for Vanilla PINN, "
    ...
    parser.add_argument("--n-b-each", type=int, default=100)
    parser.add_argument("--seed", type=int, default=5899)
    ...
    parser.add_argument("--output-name", type=str, default="poisson_residual_evolution")
    ...
    parser.add_argument(
        ...
        choices=["raw", "log"],
        default="raw",
        help="Plot raw residual magnitudes or log10 residual magnitudes.",
    )
    parser.add_argument(
        "--scale-mode",
        choices=["global", "column", "panel"],
        default="column",
        help="Use a common color scale globally, by checkpoint column, or by panel.",
    )
    parser.add_argument(
        "--clip-percentiles",
        type=float,
        ...
        metavar=("LOW", "HIGH"),
        help=(
            "Color-scale clipping percentiles. The default 0 100 uses the full range."
        ),
    )
    ...
    parser.add_argument("--lorew-eta", type=float, default=2.5e-4)
    parser.add_argument("--lorew-gamma", type=float, default=0.997)
    parser.add_argument("--kernel-h-x", type=float, default=0.05)
    parser.add_argument("--kernel-h-y", type=float, default=0.05)
    parser.add_argument("--kernel-p", type=float, default=2.0)
...
def exact_u_np(x: np.ndarray, y: np.ndarray, c: float) -> np.ndarray:
    return np.cos(c * x) + np.cos(c * y)


def source_torch(x: torch.Tensor, y: torch.Tensor, c: float) -> torch.Tensor:
...
def make_problem(cfg: Config) -> Dict[str, np.ndarray]:
    ...
    Exact = exact_u_np(X, Y, cfg.c)
    X_star = np.hstack((X.flatten()[:, None], Y.flatten()[:, None]))

    lb = np.array([cfg.x_min, cfg.y_min])
    ub = np.array([cfg.x_max, cfg.y_max])
    X_f = lb + (ub - lb) * latin_hypercube(2, cfg.n_f, cfg.seed)

    s_left = latin_hypercube(1, cfg.n_b_each, cfg.seed + 1)
    s_right = latin_hypercube(1, cfg.n_b_each, cfg.seed + 2)
    s_bottom = latin_hypercube(1, cfg.n_b_each, cfg.seed + 3)
    s_top = latin_hypercube(1, cfg.n_b_each, cfg.seed + 4)
    X_left = np.hstack(
        (
            np.full((cfg.n_b_each, 1), cfg.x_min),
            cfg.y_min + (cfg.y_max - cfg.y_min) * s_left,
        )
    )
    X_right = np.hstack(
        (
            np.full((cfg.n_b_each, 1), cfg.x_max),
            cfg.y_min + (cfg.y_max - cfg.y_min) * s_right,
        )
    )
    X_bottom = np.hstack(
        (
            cfg.x_min + (cfg.x_max - cfg.x_min) * s_bottom,
            np.full((cfg.n_b_each, 1), cfg.y_min),
        )
    )
    X_top = np.hstack(
        (
            cfg.x_min + (cfg.x_max - cfg.x_min) * s_top,
            np.full((cfg.n_b_each, 1), cfg.y_max),
        )
    )
    X_b = np.vstack((X_left, X_right, X_bottom, X_top))
    u_b = exact_u_np(X_b[:, 0:1], X_b[:, 1:2], cfg.c)
...
class BaseRunner:
    ...
    def __init__(self, cfg: Config, device: torch.device, X_b, u_b, X_r):
        self.cfg = cfg
        self.device = device
        self.iter = 0
    ...
    def net_r(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        u_xx = grad(u_x, x)
        u_yy = grad(u_y, y)
        return -u_xx - u_yy + (self.cfg.c**2) * u - source_torch(x, y, self.cfg.c)
...
class SAPINNRunner(BaseRunner):
    ...
    def __init__(self, cfg: Config, device: torch.device, X_b, u_b, X_r):
        super().__init__(cfg, device, X_b, u_b, X_r)
        ...
        self.optimizer2 = torch.optim.Adam(
            [self.lamr, self.lamb], lr=cfg.sa_weight_lr, maximize=True
        )
    ...
    def train_step(self) -> None:
        ...
        loss_b = torch.mean((self.lamb * (u_b_pred - self.u_b)) ** 2)
        loss = loss_r + loss_b
        loss.backward()
        self.iter += 1
        self.optimizer.step()
...
class LoReWPINNRunner(BaseRunner):
    ...
    def __init__(self, cfg: Config, device: torch.device, X_b, u_b, X_r):
        ...
        centers_x = torch.linspace(cfg.x_min, cfg.x_max, cfg.num_kernel_centers_x, device=device)
        centers_y = torch.linspace(cfg.y_min, cfg.y_max, cfg.num_kernel_centers_y, device=device)
...
def plot_residual_evolution(
    ...
    fig_width = max(5.0 * n_cols, 9.0)
    fig_height = max(3.35 * n_rows, 6.0)
    ...
    for r_idx, (method_name, method_residuals) in enumerate(all_residuals.items()):
    ...
    np.savez_compressed(
        ...
        method_names=np.array(list(all_residuals.keys())),
        x=problem["x0"].flatten(),
        y=problem["y0"].flatten(),
        exact=problem["Exact"],
        residual_maps=np.array(
            [[all_residuals[name][step] for step in checkpoints] for name in all_residuals.keys()],
            dtype=np.float64,
        ),
...
def main() -> None:
    ...
    for method_name, runner_cls in runner_specs:
        ...
        runner = runner_cls(cfg, device, problem["X_b"], problem["u_b"], problem["X_f"])
